...
vec2 coord(in vec2 p) {
    ...
    if (u_resolution.x > u_resolution.y) {
        ...
        p.y += (u_resolution.x - u_resolution.y) / u_resolution.y / 2.0;
    }
    p -= 0.5;
    p *= vec2(-1.0, 1.0);
    return p;
...
export const ShapeBlur: React.FC<ShapeBlurProps> = ({
  ...
  useEffect(() => {
    ...
    let w = mount.clientWidth || 100;
    let h = mount.clientHeight || 100;
    ...
    const resize = () => {
      ...
      w = mount.clientWidth || 100;
      h = mount.clientHeight || 100;
  ...
  useEffect(() => {
    ...
    mat.uniforms.u_color.value.set(c.r, c.g, c.b);
