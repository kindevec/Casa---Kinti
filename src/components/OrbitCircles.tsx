...
export interface OrbitImagesProps {
  ...
  altPrefix?: string;
  shape?: 'circle' | 'ellipse' | 'square' | 'rectangle' | 'triangle' | 'star' | 'heart' | 'infinity' | 'custom';
  customPath?: string;
  baseWidth?: number;
  radiusX?: number;
  radiusY?: number;
...
export default function OrbitImages({
  ...
  useEffect(() => {
    ...
    const step = (now: number) => {
      const dt = Math.min((now - lastTime) / 1000, 0.1); // Clamp dt to prevent tab-switch leaps
      lastTime = now;
      if (!paused && duration > 0) {
        const deltaProgress = (dt / duration) * 100 * (direction === 'reverse' ? -1 : 1);
        progress.set((((progress.get() + deltaProgress) % 100) + 100) % 100);
      }
  ...
  const containerHeight = responsive ? (height !== 'auto' ? height : undefined) : (typeof height === 'number' ? height : (typeof width === 'number' ? width : 'auto'));
  const effectiveAspectRatio = aspectRatio || (shape === 'ellipse' ? `${(radiusX * 2.2) / (radiusY * 2.6)} / 1` : (responsive ? '1 / 1' : undefined));
      ...
      >
        ...
        >
          {showPath && (
            ...
            >
              <path d={path} fill="none" stroke={pathColor} strokeWidth={pathWidth / (scale ?? 1)} strokeDasharray="6 6" />
            </svg>
          )}
