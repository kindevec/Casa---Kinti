...
export const AnimatedTabBar: React.FC<AnimatedTabBarProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Botón Pelotita / Circulito Flotante: Dorado con Ícono en Negro */}
      <motion.div
        className="absolute top-[2px] z-20 w-[52px] h-[52px] rounded-full flex items-center justify-center cursor-pointer overflow-hidden shadow-[0_8px_24px_rgba(0,0,0,0.4),0_0_15px_rgba(212,178,111,0.5)] ring-2 ring-[#0A1C24]/85"
        style={{
      ...
      >
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/60 to-transparent animate-pulse pointer-events-none" />
        ...
        <motion.div
          ...
          className="relative z-10 text-[#0A1C24] [&_svg]:stroke-current [&_svg]:w-6 [&_svg]:h-6 [&_svg]:stroke-[2.4] flex items-center justify-center"
        >
          {activeItem?.icon}
        </motion.div>
      </motion.div>
        ...
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              >
                <div className="w-6 h-6 text-[#133238] group-hover:text-[#0A1C24] transition-colors duration-200 flex items-center justify-center [&_svg]:w-5 [&_svg]:h-5 [&_svg]:stroke-current [&_svg]:stroke-[2.2]">
                  {item.icon}
                </div>
                {item.label && (
                  <span className="text-[10px] font-bold text-[#133238] group-hover:text-[#0A1C24] transition-colors duration-200 mt-1 leading-none tracking-tight">
                    {item.label}
                  </span>
                )}
              </div>
